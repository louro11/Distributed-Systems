...
public class SupplierSecurityHandler implements SOAPHandler<SOAPMessageContext> {
	...
	private static final String KEYSTORE_PATH = "jks/";
	...
	private boolean logToSystemOut(SOAPMessageContext smc) throws Exception {
		...
		try {
			...
			if (outbound) {
				...
				String classname = Singleton.getInstance().name;
				KEY_ALIAS = classname.toLowerCase();
				
				final PrivateKey privateKey = CertUtil.getPrivateKeyFromKeyStoreResource(KEYSTORE_PATH + classname + ".jks",KEYSTORE_PASS.toCharArray(),
						KEY_ALIAS,KEY_PASS.toCharArray());
                nameElement.addTextNode(classname);
				...
				//name service was all in lower case, hack job to fix it
				char[] arr = name.toCharArray();
		        arr[0] = Character.toUpperCase(arr[0]);
		        arr[4] = Character.toUpperCase(arr[4]);
		        String newname = new String(arr);
		        
		        CAClient cli = new CAClient("http://sec.sd.rnl.tecnico.ulisboa.pt:8081/ca");
		        
		        String certificateName = cli.getCertificate(newname);
