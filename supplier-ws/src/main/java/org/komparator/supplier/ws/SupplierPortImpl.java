...
public class SupplierPortImpl implements SupplierPortType {
	...
	public SupplierEndpointManager getEndPointManager() {return endpointManager; }
	...
	public List<ProductView> searchProducts(String descText) throws BadText_Exception {
		if (descText == null)
			throwBadText("Product Text cannot be null!");
		descText = descText.trim();
		if (descText.length() == 0)
			throwBadText("Product Text cannot be empty or whitespace!");
		...
		for(ProductView prod : pvs){
			if(prod.getDesc().contains(descText)){
				result.add(prod);
			}
		}
	...
	public String buyProduct(String productId, int quantity)
		...
		if (supplier.getProduct(productId) == null)
			throwBadProductId("Product identifier does not exist!");
		
		try {
			String pid = supplier.buyProduct(productId, quantity);
			
			return pid;
		} catch (QuantityException e) {
			throwInsufficientQuantity("Not enought quantity available!");
		}
		return null;
	}
